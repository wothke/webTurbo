/* 
 * Stripped down version of: Mednafen - Multi-system Emulator
 *
 ...
 * Notes: 
 *
 * For the purpose of using the PCE emulator just for the playback of 
 * HES files, I stripped down the original mednafen.cpp and commented all those
 * things out which are clearly irrelevant to the HES (I removed config file and
 * CD, VIDEO stuff, etc). I left the respective commented sections in, so that 
 * any merge activities with future updates of the original codebase might be 
 * easier. (I added nothing.. just removed.. see adapter.cpp for my additions..)
 */
...
/*
  ...
  { "filesys.fname_state", MDFNSF_CAT_PATH, gettext_noop("Format string for state filename."), fname_extra, MDFNST_STRING, "%f.%M%X" },
...
void MDFNI_DumpModulesDef(const char *fn)
{
 ...
 fp.print_format("some emu version\n" );
...
*/
static MDFN_COLD void LoadCommonPost(const char* path)	// EMSCRIPTEN: actually needed to play more than 1 song with same player
{
...
/* EMSCRIPTEN forget the settings
         {
	  std::string modoverride_settings_file_path = MDFN_GetBaseDirectory() + std::string(PSS) + std::string(MDFNGameInfo->shortname) + std::string(".cfg");
	  MDFN_LoadSettings(modoverride_settings_file_path.c_str(), true);
         }
...
//	MDFN_InitFontData();		// EMSCRIPTEN
	...
	for(unsigned int i = 0; i < MDFNSystems.size(); i++)
	{
	 ...
	 if(!MDFNSystems[i]->soundchan)
	  fprintf(stderr, "0 sound channels for %s????\n", sysname);
...
*/
  ...
  if(multiplier_save != 1)
  {
   ...
   if(MDFNGameInfo->soundchan == 2)
    ...
    if(avail > 0)
    {
  //   fprintf(stderr, "ff_resampler.avail() > espec->SoundBufMaxSize * MDFNGameInfo->soundchan - %d\n", avail);
     ff_resampler.clear();
    }
   }
  }
  ...
  // TODO: Optimize this.
  if(MDFNGameInfo->soundchan == 2 /*&& MDFN_GetSettingB(std::string(std::string(MDFNGameInfo->shortname) + ".forcemono"))*/) // EMSCRIPTEN
  {
