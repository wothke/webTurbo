...
#include "huc.h"
#include "pcecd.h"
//#include <mednafen/cdrom/scsicd.h> EMSCRIPTEN
#include "hes.h"
#include "debug.h"
#include "tsushin.h"
//#include <mednafen/hw_misc/arcade_card/arcade_card.h> EMSCRIPTEN
#include <mednafen/mempatcher.h>	// used for HES?
//#include <mednafen/cdrom/cdromif.h>	EMSCRIPTEN
...
{
 if(fp->ext != "hes" /* && fp->ext != "pce" && fp->ext != "sgx"*/)	// EMSCRIPTEN
  return false;
...
static MDFN_COLD void LoadCommonPre(void)
{
 ...
 PCE_ACEnabled = false; // MDFN_GetSettingB("pce.arcadecard"); EMSCRIPTEN
...
{ 
 IsSGX |= 0; // MDFN_GetSettingB("pce.forcesgx") ? 1 : 0; EMSCRIPTEN

 if(IsHES)
  IsSGX = 1;
 // Don't modify IsSGX past this point.
 const uint32 vram_size = 32768; // better 65536? MDFN_GetSettingUI("pce.vramsize"); EMSCRIPTEN

 vce = new VCE(IsSGX, vram_size);
 vce->SetVDCUnlimitedSprites(true /* MDFN_GetSettingB("pce.nospritelimit") EMSCRIPTEN */);
 ...
 {
  int psgrevision = PCE_PSG::REVISION_HUC6280A; // MDFN_GetSettingI("pce.psgrevision"); EMSCRIPTEN
 ...
 if(!IsHES)
 {
  MDFNGameInfo->nominal_height = 232; // MDFN_GetSettingUI("pce.slend") - MDFN_GetSettingUI("pce.slstart") + 1; EMSCRIPTEN
  MDFNGameInfo->nominal_width = 320; // MDFN_GetSettingB("pce.h_overscan") ? 320 : 288; EMSCRIPTEN

  MDFNGameInfo->lcm_width = 1120; // MDFN_GetSettingB("pce.h_overscan") ? 1120 : 1024; EMSCRIPTEN
  MDFNGameInfo->lcm_height = MDFNGameInfo->nominal_height;
 }

 vce->SetShowHorizOS(0 /*MDFN_GetSettingB("pce.h_overscan") EMSCRIPTEN */); 
...
{
	fprintf(stderr, "ERROR LoadCD should be unused\n" );	// EMSCRIPTEN
	/*
 try
 {
...
{
fprintf (stderr, "ERROR: SetMedia should not be needed\n");		// EMSCRIPTEN
/* 
...
static bool SetSoundRate(double rate)
{
 ...
 if(rate > 0)
 {
  HRRes = new OwlResampler(PCE_MASTER_CLOCK / 12, rate, 0.0000350 /*MDFN_GetSettingF("pce.resamp_rate_error")*/, 20, 3/*MDFN_GetSettingUI("pce.resamp_quality")*/);	// EMSCRIPTEN XXX useful?
