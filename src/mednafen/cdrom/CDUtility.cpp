...
#include "CDUtility.h"
//#include "dvdisaster.h"	// EMSCRIPTEN
//#include "lec.h"
...
fprintf(stderr, "ERROR: encode_mode0_sector\n");
...
bool edc_check(const uint8 *sector_data, bool xa)
{
 ...
 return true; // EMSCRIPTEN(CheckEDC(sector_data, xa));
}
...
bool edc_lec_check_and_correct(uint8 *sector_data, bool xa)
{
 ...
 return true; //EMSCRIPTEN(ValidateRawSector(sector_data, xa));
}
